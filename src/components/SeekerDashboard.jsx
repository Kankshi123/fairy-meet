...
import { Heart, MessageCircle, Phone, Search, Bell, Settings, LogOut, LayoutGrid, Layers, X as XIcon } from 'lucide-react';
...
const SwipeCard = ({ comp, onSwipe, isTop }) => {
      ...
      <div className="p-6 flex flex-col h-[35%] justify-between pointer-events-none">
        <div>
          <div className="flex justify-between items-start mb-2">
            ...
            <span className="text-sm font-medium text-vibrant-pink bg-off-white px-3 py-1 rounded-md">{comp.rate}</span>
          </div>
...
export default function SeekerDashboard({ onInitiateContact, user, onLogout }) {
    ...
    <div className="min-h-screen bg-off-white font-sans text-rich-black pb-24 flex flex-col">
      ...
      <header className="bg-white/80 backdrop-blur-md sticky top-0 z-40 border-b border-rich-black/10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            <div className="hidden sm:flex items-center gap-2 bg-off-white px-4 py-2 rounded-full">
              <span className="text-xs uppercase tracking-widest font-semibold text-rich-black/60">Wallet</span>
              <span className="font-medium text-rich-black">₹{user.walletBalance}</span>
            </div>
            ...
            <div className="w-10 h-10 rounded-full bg-vibrant-pink flex items-center justify-center text-white font-serif text-lg">
      ...
      <main className="max-w-7xl w-full mx-auto px-6 pt-8 flex-grow flex flex-col">
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="font-serif text-4xl text-rich-black mb-2">Discover Connections</h1>
            <p className="text-rich-black/60 text-lg">Find someone who matches your vibe.</p>
          </div>
                ...
                >
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className="text-xs font-medium text-vibrant-pink bg-off-white px-2 py-1 rounded-md">{comp.rate}</span>
                    </div>
                    ...
                    <div className="flex gap-3 mt-auto">
                      <button 
                        onClick={() => onInitiateContact('chat', comp.name)}
                        className="flex-1 flex items-center justify-center gap-2 bg-off-white text-rich-black py-2.5 rounded-xl hover:bg-vibrant-pink hover:text-white transition-colors"
                      >
                        <MessageCircle className="w-4 h-4" />
                        <span className="text-sm font-medium">Chat</span>
                      </button>
                      <button 
                        onClick={() => onInitiateContact('call', comp.name)}
                        className="flex-1 flex items-center justify-center gap-2 bg-rich-black text-pure-white py-2.5 rounded-xl hover:bg-rich-black transition-colors"
                      >
